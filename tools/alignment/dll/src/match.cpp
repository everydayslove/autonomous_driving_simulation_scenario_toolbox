...
//		cv::sort(temp_dis, sort_dis, SORT_EVERY_ROW + SORT_ASCENDING);//升序排列
...
/*image_1表示参考图像，
 image_2表示待配准图像
 dmatches表示最近邻和次近邻匹配点对
 keys_1表示参考图像特征点集合
 keys_2表示待配准图像特征点集合
 model表示变换模型
 right_matches表示参考图像和待配准图像正确匹配点对
	       ...
	       vector<KeyPoint>& keys_2, const string& model,vector<DMatch> &right_matches,double& end_match_time)
{
	...
	for (size_t i = 0; i < dmatches.size(); i++)
	{
		...
		if ((dis_1 / dis_2) < dis_ratio)//如果满足距离比关系
		{
			...
			init_matchs.push_back(dmatches[i][0]);//保存正确的dmatches_pos
		}
		//else
		//{
		//	point_1.push_back(keys_1[dmatches[i][0].queryIdx].pt);
		//	point_2.push_back(keys_2[dmatches[i][0].trainIdx].pt);
		//	init_matchs.push_back(dmatches[i][0]);//直接保存的dmatches_pos
		//}
	}

//#ifdef _DEBUG
	//findHistogram(distances);
	//std::cout << "距离比之后初始匹配点对个数是： " << init_matchs.size() << "\n";
//#endif 
...
#ifdef _DEBUG
	//cout << "使用RANSAC删除错误点对后,正确匹配个数： " << right_matches.size() << "\n";
#endif
	//if (rmse > 0.7)
	//{
		//cout << "误差rmse: " << rmse << "\n";
